...
int addVal(string);
void prntH(string);
bool play(string[],string&, string&, string&, int&, int&, float&);
...
int main(int argc, char** argv) {
    ...
    do {
        ...
        n2Shufl = 20; //How many times to Shuffle the Deck
        deck[NUMCRDS-1]; //52 Cards represented as a string Face|Suit
        ...
        while (!bust) {
            ...
            bust = play(deck, dealer,player, header2, count, value2, bet);
            turn++;
            if (bust) {
        ...
        if (answer == 'y') {
           ...
           // deck[NUMCRDS];
            dealer="";
             player="";
            index = 0;
            value1 = 0; //Dealer's total card values
             value2 = 0; //Player's total card values
            plays = 0; //Total plays. When a player "busts" it is considered a play.

             count = 0;
           j = 2;
            k = 0;
             totSplit = 0; //Counts the number of splits. Only 3 are allowed in Black Jack.
...
bool play(string c[],string& dealer, string& hand, string& header, int& count, int& value, float& bet) {
    char answer;
    bool x = false;
    int k = 0;
    for (int i = 0;i < 51;i++) {
...
void calcVal(vector<int> vect, int bet) {
    for (int i = 1; i < vect.size(); i++) {
        if (vect[0] > vect[i] || vect[i] > 21) {
            ...
            cout << "Player's Hand " << i << " neither won nor lost" << endl;
        }
    }
}
void calcProb() {
